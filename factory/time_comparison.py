...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '--alg', help='algorithm to use', type=lambda v: Algorithm[v], choices=list(Algorithm),
                        required=True)
    ...
    times, aik_s = measure_time_for_ai_e_n_half(max_n=args.max_n, sample_size=args.sample_size)
    print(times)
    print()
    print(aik_s)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    file_path = os.path.join(script_dir, f"results/{args.alg.value}/times.csv")
    times.to_csv(file_path, index=False)

    file_path = os.path.join(script_dir, f"results/{args.alg.value}/ai_ks.csv")
