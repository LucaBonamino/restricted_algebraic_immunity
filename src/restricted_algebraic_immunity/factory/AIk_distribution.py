...
class AIkDistribution:
    ...
    def run_immunity(sub_tt: List[int], slice_domain: List[int], n_vars: int, algorithm: Algorithm,
        ...
        if algorithm == Algorithm.IV:
            if padded_sub_tt is True:
                ...
                dt = time.time() - t
            _log.info(f"AIk per sample: {aik}")
        else:
    ...
    @staticmethod
    def plot_aik_average(average_dataframe, sample_size: int, m: int, n: int, parallelization_type: str,
                         save: bool = True):
        fig, ax = plt.subplots()
...
def main(n: int, k_max: int, k_min: int, parallelize_by: ParallelizationType, sample_size: int, plot: bool,
    ...
    save_raw_data_to_file(filename=file_path, data=str(times_df))
    print(dist_df)
    dist_latex = dist_df
    if plot is True:
    ...
    save_raw_data_to_file(filename=file_path, data=str(dist_latex))

    file_path = dire / f"{file_name_prefix}_averages.txt"
    # averages_latex = df_averages.to_latex(index=False, escape=False)
    save_raw_data_to_file(filename=file_path, data=str(df_averages))
